...
vector<int> max(9, 0);    //record the highest score of the total nine courses;
vector<int> min(9, 0);    //record the lowest score of the total nine courses;
...
int main() {
		...
		if (student.name != "%") {   // the whole input ends up with '%';
			...
			for (j = 0; j < 1000; j++) {
				...
				if (student.course[j] != "%") {   //each student's information ends up with '%';
					...
					if (max[courseNum] == 0) {   //find the highest score of this course;
						max[courseNum] = sco;
					}
					else if (student.score[j] > max[courseNum]) {
						max[courseNum] = sco;
					}
					if (min[courseNum] == 0) {    //find the lowest score of this course;
						min[courseNum] = sco;
					}
					else if (student.score[j] < min[courseNum]) {
						min[courseNum] = sco;
					}
	...
	cout << setw(10) << setfill(' ') << left << max[0];
	cout << setw(9) << setfill(' ') << left << max[1];
	cout << setw(10) << setfill(' ') << left << max[2];
	cout << setw(10) << setfill(' ') << left << max[3];
	cout << setw(12) << setfill(' ') << left << max[4];
	cout << setw(10) << setfill(' ') << left << max[5];
	cout << setw(10) << setfill(' ') << left << max[6];
	cout << setw(11) << setfill(' ') << left << max[7];
	cout << max[8] << endl;

	cout << "    Min      "; // output each course's lowest score;
	cout << setw(10) << setfill(' ') << left << min[0];
	cout << setw(9) << setfill(' ') << left << min[1];
	cout << setw(10) << setfill(' ') << left << min[2];
	cout << setw(10) << setfill(' ') << left << min[3];
	cout << setw(12) << setfill(' ') << left << min[4];
	cout << setw(10) << setfill(' ') << left << min[5];
	cout << setw(10) << setfill(' ') << left << min[6];
	cout << setw(11) << setfill(' ') << left << min[7];
	cout << min[8] << endl;
    system("pause");
	return 0;
